...
class Player(pygame.sprite.Sprite):
    ...
    def super_shoot(self):
        if self.super_energy > 0:
            if self.stripe == 1:
                s_bullet = S_bullet(self.rect.left - 3, self.rect.centery,self.stripe)
                all_sprites.add(s_bullet)
                bullets1.add(s_bullet)
            if self.stripe == 2:
                s_bullet = S_bullet(self.rect.right, self.rect.centery,self.stripe)
                all_sprites.add(s_bullet)
...
def play():
    ...
    while running:
        ...
        clock.tick(FPS)
        hits = pygame.sprite.groupcollide(players, mobi, False,  1-shield)
